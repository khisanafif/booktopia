...
if(isset($_GET['c']) && isset($_GET['s'])){
    $cat_qry = $conn->query("SELECT * FROM kategori where md5(id) = '{$_GET['c']}'");
    if($cat_qry->num_rows > 0){
        $result =$cat_qry->fetch_assoc();
        $title = $result['category'];
        $cat_description = $result['description'];
    }
 $sub_cat_qry = $conn->query("SELECT * FROM sub_kategori where md5(id) = '{$_GET['s']}'");
    ...
    <div class="container-fluid row">
        ...
        <div class="col-md-3 border-right mb-2 pb-3">
            ...
            <div class="list-group">
                ...
                <?php 
                $sub_cat = $conn->query("SELECT * FROM `sub_kategori` where md5(parent_id) =  '{$_GET['c']}' ");
                ...
                <div class="tab-pane fade show active" id="book">
                    ...
                    <div class="row gx-2 gx-lg-2 row-cols-1 row-cols-md-3 row-cols-xl-3">
                        ...
                        <?php 
                            ...
                            $products = $conn->query("SELECT * FROM `produk` where status = 1 {$whereData} order by rand() ");
                            while($row = $products->fetch_assoc()):
                                ...
                                $inventory = $conn->query("SELECT * FROM gudang where id_produk = ".$row['id']);
                                $inv = array();
                                while($ir = $inventory->fetch_assoc()){
                                    $inv[] = number_format($ir['harga']);
                                }
                        ?>
