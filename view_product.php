<?php 
 $products = $conn->query("SELECT * FROM `produk`  where md5(id) = '{$_GET['id']}' ");
 if($products->num_rows > 0){
    ...
    $inventory = $conn->query("SELECT * FROM gudang where product_id = ".$id);
    $inv = array();
...
<section class="py-5 bg-light">
    <div class="container px-4 px-lg-5 mt-5">
        ...
        <?php 
            $products = $conn->query("SELECT * FROM `produk` where status = 1 and (id_kategori = '{$category_id}' or id_sub_kategori= '{$sub_category_id}') and id !='{$id}' order by rand() limit 4 ");
